...
public class TicTacToeGame {
	...
	*/
	...
	private Transformation[] transCarre /*= {Transformation.ID,Transformation.ROT,Transformation.ROT,Transformation.ROT,Transformation.HSYM,Transformation.ROT,Transformation.ROT,Transformation.ROT}*/;
	private Transformation[] NontransCarre /*= {Transformation.ID,Transformation.HSYM,Transformation.VSYM,Transformation.HSYM}*/;
    private int posSym /*= 0*/;
    ...
    *  the number of cells that must be aligned to win.
	...
	public TicTacToeGame(int lines, int columns, int sizeWin){
        ...
        this.transCarre = {Transformation.ID,Transformation.ROT,Transformation.ROT,Transformation.ROT,Transformation.HSYM,Transformation.ROT,Transformation.ROT,Transformation.ROT};
		this.NontransCarre = {Transformation.ID,Transformation.HSYM,Transformation.VSYM,Transformation.HSYM};
		this.posSym = 0;
	}
    ...
    *  String representation of the game
    ...
    public void reset(){
		this.posSym = -1;
		this.transformedBoard = new int[this.lines*this.columns];
		if (this.lines == this.columns)
		   //Si c'est carre; on fait une matrice de longueur transCarre;sinon;on utilise l'autre tableau NonTransCarre
		   // a chaque sous liste de la matrice; correspond le type de symetrie applique selon la sequence dans transCarre/NontransCarre
		   this.transform = new int[transCarre.length][this.lines*this.columns];
		else
		   this.transform = new int[NontransCarre.length][this.lines*this.columns];


		for (int e = 0; e < this.lines*this.columns;e++){
			this.transformedBoard[e] = e;// le reset;on remet a l'ordre normale des indices; aucune symetrie appliquee
		}
    }
     ...
     *
     * @return
     *   true if there are additional symmetries
     */
    public boolean hasNext(){
        // posSym s'incremente a chaque fois qu'on applique un type de symetrie differente;si le posSym
		//depasse l'index admissible dans transCarre/NonTransCarre;cela veut dire qu'on a itere a travers toutes
		//les symetries possibles pour le cas donne
		if (this.lines == this.columns){
			return transCarre.length - 1 > this.posSym;
		}
		else{
           return NontransCarre.length - 1> this.posSym;
		}
    }
    ...
    public void next(){
		...
		if (this.hasNext() == true){
          ...
          if (this.lines == this.columns)
			...
			T = NontransCarre[++posSym]; 
		
		  switch (T){
			case ROT:Utils.rotate(this.lines,this.columns,this.transformedBoard);break;
			case HSYM:Utils.horizontalFlip(this.lines,this.columns,this.transformedBoard);break;
			case VSYM:Utils.verticalFlip(this.lines,this.columns,this.transformedBoard);break;
		  }
		  System.arraycopy(transformedBoard,0,transform[posSym],0,transformedBoard.length);
		}
  	...
  	*/    
  	public boolean equalsWithSymmetry(TicTacToeGame other){
			...
			while (this.hasNext()){
				...
				boolean exist = true;
				for (int e = 0; e < this.lines*this.columns;e++){
					if (other.board[e] != this.board[transformedBoard[e]]){
					   exist = false;
                       break;
					}
				}
